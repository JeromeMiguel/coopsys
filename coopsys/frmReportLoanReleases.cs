...
namespace coopsys
{
    public partial class frmReportLoanReleases : MetroFramework.Forms.MetroForm
    {
        DataTable dt = new DataTable();
        DataCollection dc = new DataCollection();
        MySqlConnection conn;
        public string query, typeColumn, typeField, path;
        bool retry = true;
        public string year;
        ...
        public frmReportLoanReleases(MySqlConnection _conn)
        {
            conn = _conn;
            InitializeComponent();
        ...
        private void frmReportLoanReleases_Load(object sender, EventArgs e)
        {
        ...
        {
            if (cboType.Text == "Gross Loan")
            {
                typeColumn = "loanamount";
                typeField = "AMOUNT";

            }
            else if (cboType.Text == "Net Loan")
            {
                typeColumn = "loancomputed";
                typeField = "AMOUNT";

            }

            query = "SELECT CONCAT( lastname,', ', firstname, ' ', middlename) as 'NAME OF MEMBER', FORMAT(SUM(`" + typeColumn + "`),2) as '" + typeField + "' " +
                "from loan inner join member on member.memberid = loan.memberid where loanyear = " + cboYear.Text + " GROUP BY loan.memberid ORDER BY 'NAME OF MEMBER' ASC";
            ...
            lblTotal.Text = "₱" + dc.fnReturnStringValue("SELECT FORMAT(SUM(`" + typeColumn + "`),2) as \"TOTAL LOAN\" from loan where loanyear = " + cboYear.Text + ";", "TOTAL LOAN", conn);

            grdReportLoanReleases.DataSource = dt;
            if (grdReportLoanReleases.Rows.Count > 0)
            {
                btnExportExcel.Enabled = true;
            }
            grdReportLoanReleases.Columns[2].Visible = false;

            foreach (DataGridViewRow row in grdReportLoanReleases.Rows)
            {
                row.HeaderCell.Value = String.Format("{0}", row.Index + 1);
            }
        }

        private void grdReportLoanReleases_ColumnHeaderMouseClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            foreach (DataGridViewRow row in grdReportLoanReleases.Rows)
            {
                row.HeaderCell.Value = String.Format("{0}", row.Index + 1);
        ...
        {
            grdReportLoanReleases.DataSource = null;
            btnExportExcel.Enabled = false;
            cboType.SelectedIndex = 0;
            cboYear.SelectedIndex = 0;
            lblTotal.Text = "₱0.00";
        ...
        private void bgWorker_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            wSheet.Cell("A4").Value = "List of Cooperative Members with their Loan Releases";
            ...
            for (int c = 0; c < dt.Columns.Count; c++)
            {
                if (c + 1 < dt.Columns.Count)
                {
                    ...
                    wSheet.Cell(col).Value = grdReportLoanReleases.Columns[c].HeaderText.ToString();
            ...
            for (int r = 0; r < dt.Rows.Count; r++)
            {
                for (int c = 0; c < dt.Columns.Count; c++)
                {
                    if (c == 2)
                    {
                        wSheet.Cell(Letters[0] + (r + 10)).Value = int.Parse(grdReportLoanReleases.Rows[r].Cells[c].Value.ToString());
                    }

                    else if (c == 1)
                    {

                        decimal loanValue = decimal.Parse(grdReportLoanReleases.Rows[r].Cells[c].Value.ToString());
                        wSheet.Cell(Letters[c + 1] + (r + 10)).Value = loanValue;
                    }
                    else
                    {
                        wSheet.Cell(Letters[c + 1] + (r + 10)).Value = grdReportLoanReleases.Rows[r].Cells[c].Value.ToString().ToUpper();
                    }
                }
            ...
            //Total
            wSheet.Cell("B" + (dt.Rows.Count + 11)).Value = "Total";
            wSheet.Cell("C" + (dt.Rows.Count + 11)).FormulaA1 = "SUM( C10:C" + (dt.Rows.Count + 9) + ")";
            ...
            //workSheet["A4"].Value = "List of Cooperative Members with their Loan Releases";
            ...
            //workSheet["A5"].Value = "For Year " + cboYear.Text + "";
            ...
            //    {
            //        string col = Letters[c + 1] + (7);
            //        workSheet[col].Value = grdReportLoanReleases.Columns[c].HeaderText.ToString();
            ...
            //        if (c == 2)
            //        {
            //            workSheet[Letters[0] + (r + 10)].Value = int.Parse(grdReportLoanReleases.Rows[r].Cells[c].Value.ToString());
            //        }
            ...
            //        else
            //        {
            //            workSheet[Letters[c + 1] + (r + 10)].Value = grdReportLoanReleases.Rows[r].Cells[c].Value.ToString().ToUpper();
            //        }
            //    }
            ...
            //workSheet[Letters[2] + (dt.Rows.Count + 11)].Value = decimal.Parse(dc.fnReturnStringValue("SELECT FORMAT(SUM(`" + typeColumn + "`),2) as \"TOTAL LOAN\" from loan where loanyear = 2024;", "TOTAL LOAN", conn));
